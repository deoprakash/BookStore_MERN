...
import { ShoppingBag, Plus, Minus, Star, Search, Type } from "lucide-react"
...
const Books = () => {
    ...
    { id: 16, title: "The Unwilling", author: "John Hart", price: 399, description: "The adventures of a nobleman who imagines himself a knight...", image: BP16 }
     ...
     const filteredBooks = books.filter(book => {
          ...
          const matchSearch = searchTerm === "" || book.title.toLocaleLowerCase(lowerSearch) || book.author.toLowerCase().includes(lowerSearch)
          ...
          <div className={styles.container}>
               <div className={styles.innerContainer}>
                    <div className={styles.headerWrapper}>
                        ...
                        <p className={styles.headerSubtitle}>Explore our curated collection spannig geners and perspective.</p>
                    </div>
                                   ...
                                   <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}
                                        className={styles.selectBox}>
                                             {categories.map((category) => (
                                                  <option value={category} key={category}>{category === "all" ? "All Genres": category}</option>
                                             ))}
                                   </select>
